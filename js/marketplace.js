...
function createOrder({ productId, qty }) {
  ...
  const id =
    "o_" + Date.now().toString(36) + Math.random().toString(16).slice(2, 6);
...
function proposeMeetup(orderId, pointId, dateTimeISO) {
  ...
  if (u.id !== o.buyerId && u.id !== o.sellerId)
    return { ok: false, msg: "Sin permisos." };
...
function acceptMeetup(orderId) {
  ...
  if (o.status !== "proposed")
    return { ok: false, msg: "No hay propuesta por aceptar." };
  if (u.id !== o.buyerId && u.id !== o.sellerId)
    return { ok: false, msg: "Sin permisos." };
...
function markDelivered(orderId) {
  ...
  if (u.id !== o.sellerId)
    return { ok: false, msg: "Solo el vendedor puede marcar entregado." };
