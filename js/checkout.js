...
function renderStep() {
  [stepPill1, stepPill2, stepPill3].forEach((p, i) => {
    if (i + 1 === currentStep) p.setAttribute("aria-current", "step");
...
function validateInvoiceFields() {
  ...
  if (!invoiceName || !invoiceRfc || !invoiceEmail) {
    setMsg(
      "Completa los datos de facturación o desactiva la opción de factura.",
      true,
    );
    return false;
  }
...
function createOrderAndContinue() {
  ...
  const order = {
    ...
    buyerName:
      currentUser()?.name || document.querySelector("#fullName").value.trim(),
    folio,
